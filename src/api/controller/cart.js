...
module.exports = class extends Base {
  ...
  async addAction() {
    ...
    const { status, msg } = await this.addCart(goodsId, productId, number);
    if (status === 200) {
      return this.success(this.getCart());
    } else {
      return this.fail(status, msg);
    }
  }
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifition = await this.model('goods_specification')
          .field([
            ...
            'nideshop_specification.name'
          ])
          .join(
            'nideshop_specification ON nideshop_specification.id=nideshop_goods_specification.specification_id'
          )
          .where({
  ...
  async checkOneAction() {
    ...
    if (status === 400) {
      ...
      const checkedGoodsList = cartData.cartList.filter(function(v) {
        return v.product_id + '' === productId && v.goods_id + '' === goodsId;
      });
      // 根据收货地址计算运费
